...
void Paddle::paddleReset() {
  this -> pWidth = this -> _paddleDefaulSize;
  this -> x = (WIDTH - this -> pWidth)/2;
...
void Paddle::paddleDraw() {
  ...
  gb.display.drawLine( this -> x + 2 , this -> y , this -> x + this -> pWidth - 2 , this -> y );
  gb.display.setColor(GRAY);
  gb.display.drawLine( this -> x + 2 , this -> y + 1 , this -> x + this -> pWidth - 2 , this -> y +1 );
  gb.display.drawImage(this -> x, this -> y, paddleLeftSide);
  gb.display.drawImage(this -> x + this -> pWidth - 2, this -> y, paddleRightSide);
}
